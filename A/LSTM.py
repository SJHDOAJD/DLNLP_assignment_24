...
def model_LSTM(tokenizer):
    ...
    class sentimentLSTM(nn.Module):
        def __init__(self, embedding_matrix, hidden_dim, output_size, n_layers):
            ...
            self.lstm = nn.LSTM(embed_dim,hidden_dim,n_layers,dropout=0.5,batch_first=True)
    ...
    model = sentimentLSTM(embedding_matrix, hidden_dim, output_size, n_layers)

    return model


def train_LSTM(model, train_loader, valid_loader):
    ...
    # save diagram
    plot = os.path.join(init_path, 'plot', 'LSTM accuracy and loss diagram')
    plt.savefig(plot)
    plt.close()
...
def evaluate_LSTM(model, test_loader):
    ...
    # save diagram
    plot = os.path.join(init_path, 'plot', 'LSTM confusion matrix diagram')
    plt.savefig(plot)
    plt.close()
    ...
    ax1.grid(True)
    # save diagram
    plot1 = os.path.join(init_path, 'plot', 'LSTM Performance Metrics diagram')
    plt.savefig(plot1)
    plt.close()
